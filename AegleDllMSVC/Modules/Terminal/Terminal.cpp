...
void Terminal::RenderConsole() {
    ...
    ImGui::BeginChild("##TermHeader", ImVec2(0, headerH), false, ImGuiWindowFlags_NoScrollbar);
    {
        ...
        ImGui::TextColored(ImVec4(0.55f, 0.55f, 0.65f, 1.0f), "AEGLESEEKER");
        ImGui::SameLine(0, 0);
...
void Terminal::RenderUnloadDialog() {
    ...
    if (ImGui::BeginPopupModal("Confirm Unload##UnloadDialog", nullptr,
        ...
        ImGui::TextColored(titleText, "Are you sure you want to unload Aegleseeker?");
        ImGui::Spacing();
        ...
        if (ImGui::Button("Cancel##UnloadNo", ImVec2(btnW, btnH))) {
        ...
        ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 1.0f, 1.0f, 1.0f));
        if (ImGui::Button("Yes, Unload##UnloadYes", ImVec2(btnW, btnH))) {
            g_showUnloadDialog = false;
            ImGui::CloseCurrentPopup();
