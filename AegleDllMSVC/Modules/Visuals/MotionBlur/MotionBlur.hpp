...
public:
    ...
    // Backbuffer storage (reusable frame pool to avoid per-frame D3D resource churn)
    static std::vector<ID3D11Texture2D*> g_backbufferTextures;
    static std::vector<ID3D11ShaderResourceView*> g_poolSRVs; // SRVs cached 1:1 with g_backbufferTextures
    static int g_maxBackbufferFrames;
    static int g_currentBackbufferIndex;
    static UINT g_poolWidth;
    static UINT g_poolHeight;
    static DXGI_FORMAT g_poolFormat;
    ...
    /// @brief Capture the current frame into the history pool (call from the present hook)
    static void OnPresent(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, IDXGISwapChain* pSwapChain);

    /// @brief Draw the frame-trail overlay (call once per frame inside ImGui's frame)
    static void RenderTrail(bool menuOpen);
