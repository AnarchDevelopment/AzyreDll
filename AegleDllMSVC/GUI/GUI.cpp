...
bool GUI::RenderSidebarButton(const char* label, int index) {
    ...
    // Active tab: soft accent-tinted background + subtle inner glow
    if (active && anim > 0.001f) {
        ImVec4 accentV = g_colorAccent;
        ImU32 activeBg = ImColor(accentV.x, accentV.y, accentV.z, anim * 0.13f);
        ImU32 activeGlow = ImColor(accentV.x, accentV.y, accentV.z, anim * 0.06f);
        draw->AddRectFilled(ImVec2(p_min.x - 3, p_min.y - 3), ImVec2(p_max.x + 3, p_max.y + 3), activeGlow, 11.0f);
        draw->AddRectFilled(p_min, p_max, activeBg, 8.0f);
    }
...
void GUI::RenderMenu(float screenWidth, float screenHeight) {
    ...
    // Background: dark overlay for "Normal Dark", light tint on top of the DX11 Mica blur otherwise
    if (ClickGUI::g_bgStyle == 0) {
        ...
        ImU32 tint = IM_COL32(5, 5, 10, (int)(e * 30.0f));
    ...
    if (e > 0.01f) {
        ...
        if (ImGui::Begin("Aegleseeker", NULL, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar)) {
            ...
            ImGui::BeginChild("HeaderBar", ImVec2(0, 55.0f * sc), false, ImGuiWindowFlags_NoScrollbar);
            {
                ...
                // Animated accent sweep along the separator when switching tabs
                float sweepT = Animations::EaseInOutQuad(GUI::g_tabAnim);
                if (sweepT > 0.002f && sweepT < 0.998f) {
                    float sweepX = wPos.x + sweepT * wSize.x;
                    float segLen = 90.0f * sc;
                    draw->AddLine(
                        ImVec2(sweepX - segLen * 0.35f, wPos.y + 54.0f * sc),
                        ImVec2(sweepX + segLen, wPos.y + 54.0f * sc),
                        ImColor(g_colorAccent.x, g_colorAccent.y, g_colorAccent.z, 0.25f),
                        1.5f
                    );
                    draw->AddLine(
                        ImVec2(sweepX, wPos.y + 54.0f * sc),
                        ImVec2(sweepX + segLen * 0.6f, wPos.y + 54.0f * sc),
                        ImColor(g_colorAccent.x, g_colorAccent.y, g_colorAccent.z, 0.95f),
                        1.5f
                    );
                }
            ...
            ImGui::BeginChild("ContentAreaParent", ImVec2(0, 0), false, ImGuiWindowFlags_NoScrollbar);
            {
                ...
                ImGui::Dummy(ImVec2(ImGui::CalcTextSize(tabNames[GUI::g_currentTab]).x, ImGui::GetFontSize() + 8.0f));
                ImGui::PopFont();
                ImGui::Separator();
