...
char Watermark::g_customText[128] = "Aegleseeker";
...
void Watermark::RenderDisplay() {
    if (g_showWatermark || g_watermarkAnim > 0.01f) {
        ...
        float easedAnim = Animations::EaseOutExpo(g_watermarkAnim);
        ImDrawList* draw = ImGui::GetForegroundDrawList();
        ImVec2 pos = g_watermarkHud->pos;
        ...
        if (g_useImage && g_watermarkTexture) {
            ...
            if (g_showBackground) {
                ...
                tSize.x *= (size / 16.0f); tSize.y *= (size / 16.0f);
                draw->AddRectFilled(ImVec2(pos.x - 10, pos.y - 5), ImVec2(pos.x + tSize.x + 10, pos.y + tSize.y + 5), ImColor(5, 5, 10, (int)(easedAnim * g_bgOpacity * 255.0f)), 5.0f);
            }
            ...
            if (g_showGlow) {
                ...
                for (int i = 2; i >= 1; --i) {
                    ...
                    DrawGradientText(draw, font, size, ImVec2(pos.x + i, pos.y), g_customText, Watermark::g_chromaColors, glowAlpha, g_chromaText);
                    DrawGradientText(draw, font, size, ImVec2(pos.x - i, pos.y), g_customText, Watermark::g_chromaColors, glowAlpha, g_chromaText);
                    DrawGradientText(draw, font, size, ImVec2(pos.x, pos.y + i), g_customText, Watermark::g_chromaColors, glowAlpha, g_chromaText);
                    DrawGradientText(draw, font, size, ImVec2(pos.x, pos.y - i), g_customText, Watermark::g_chromaColors, glowAlpha, g_chromaText);
                }
            }
            
            DrawGradientText(draw, font, size, pos, g_customText, g_chromaText ? Watermark::g_chromaColors : std::vector<ImVec4>{Watermark::g_staticColor}, easedAnim, g_chromaText);
...
void Watermark::RenderMenu() {
    ...
    if (GUI::BeginModuleSettings("Watermark", &g_showWatermark)) {
        ...
        if (!g_useImage) {
            ...
            ImGui::SliderFloat("Size", &g_fontSize, 12.0f, 64.0f, "%.0f px");
            ...
            if (g_chromaText) {
                ...
                ImGui::SliderFloat("Speed", &g_chromaSpeed, 0.1f, 5.0f, "%.1fx");
            ...
            if (g_showBackground) {
                ImGui::SliderFloat("BG Opacity", &g_bgOpacity, 0.0f, 1.0f, "%.2f");
            }
        } else {
            ImGui::Separator();
            ImGui::Text("Image Settings");
            ImGui::SliderFloat("Height", &g_imageSize, 10.0f, 200.0f, "%.0f px");
            ImGui::SliderFloat("Opacity", &g_imageOpacity, 0.0f, 1.0f, "%.2f");
            
            if (!g_watermarkTexture) {
                ImGui::TextColored(ImVec4(1, 0.4f, 0.4f, 1), "Image failed to load!");
                if (ImGui::Button("Retry Load")) InitializeTextures();
            } else {
                ImGui::Text("Preview:");
