...
void IRChat::RenderMenu() {
    ...
    if (!s_hasCredentials) {
        ImGui::SetCursorPosY(ImGui::GetCursorPosY() + (avail.y - 300) * 0.3f);
        ImGui::BeginChild("IRCRegister", ImVec2(avail.x * 0.7f, 320.0f), true, ImGuiWindowFlags_None);
        {
            ImVec2 innerAvail = ImGui::GetContentRegionAvail();
            ImGui::SetCursorPos(ImVec2((innerAvail.x - ImGui::CalcTextSize("IRC CHAT REGISTRATION").x) * 0.5f, 20));
            ...
            ImGui::Text("Username:");
            ImGui::SameLine(120);
            ImGui::SetNextItemWidth(innerAvail.x - 150);
            ...
            ImGui::SetCursorPosX((innerAvail.x - btnWidth) * 0.5f);
        ...
        if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("DRAG_IRC_CONFIG")) {
            ...
            if (file.is_open()) {
                ...
                if (client.SendConfigFile(cfgName, content, "")) {
                    client.PushSystemMessage("Client", "Config compartida: " + cfgName);
                } else {
                    client.PushSystemMessage("Client", "Error al compartir config.");
                }
            }
        }
    ...
    if (ImGui::IsItemHovered()) {
        ImGui::SetTooltip("Arrastra configuraciones desde el panel derecho para compartirlas.");
    }
