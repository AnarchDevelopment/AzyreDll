...
// Stable hue derived from a nick so every user keeps a consistent color.
static float NickHue(const std::string& nick) {
    unsigned h = 2166136261u;
    for (char c : nick) {
        h ^= (unsigned char)c;
        h *= 16777619u;
    }
    return (float)(h % 360) / 360.0f;
}
...
void IRChat::RenderMenu() {
    ...
    ImGui::PushStyleColor(ImGuiCol_ChildBg, ImVec4(GUI::g_colorBgPanel.x, GUI::g_colorBgPanel.y, GUI::g_colorBgPanel.z, 0.55f));
    ImGui::PushStyleColor(ImGuiCol_Border, ImVec4(GUI::g_colorAccent.x * 0.25f, GUI::g_colorAccent.y * 0.25f, GUI::g_colorAccent.z * 0.25f, 0.30f));
    ImGui::PushStyleVar(ImGuiStyleVar_WindowBorderSize, 1.0f);
    ImGui::PushStyleVar(ImGuiStyleVar_WindowRounding, 8.0f);
    ImGui::BeginChild("IRCHeader", ImVec2(0, 52.0f), true, ImGuiWindowFlags_NoScrollbar);
    {
        ImDrawList* draw = ImGui::GetWindowDrawList();
        float hw = ImGui::GetWindowWidth();

        // Channel title with glow
        ImGui::SetCursorPos(ImVec2(16.0f, 13.0f));
        ImGui::PushFont(GUI::g_fontH3 ? GUI::g_fontH3 : ImGui::GetFont());
        ImVec2 chPos = ImGui::GetCursorScreenPos();
        GUI::AddTextGlow(draw, ImGui::GetFont(), ImGui::GetFontSize(), chPos,
            ImColor(GUI::g_colorAccent.x, GUI::g_colorAccent.y, GUI::g_colorAccent.z, 1.0f), "#aeglegeneral", 3.0f);
        ImGui::Dummy(ImVec2(ImGui::CalcTextSize("#aeglegeneral").x, ImGui::GetFontSize()));
        ImGui::PopFont();
        ...
        if (status == IRCStatus::Connecting) {
            dotColor = ImColor(1.0f, 0.75f, 0.15f, 0.55f + pulse * 0.45f);
            statusText = "Connecting...";
        } else if (status == IRCStatus::Connected) {
            dotColor = ImColor(0.15f, 0.95f, 0.35f, 0.55f + pulse * 0.45f);
            statusText = "Connected as " + client.GetCurrentNick();
        } else if (status == IRCStatus::ConnectionFailed) {
            dotColor = ImColor(0.95f, 0.2f, 0.2f, 1.0f);
            statusText = "Connection Failed";
        }

        ImVec2 stSz = ImGui::CalcTextSize(statusText.c_str());
        float pillW = stSz.x + 32.0f;
        draw->AddRectFilled(pillPos, ImVec2(pillPos.x + pillW, pillPos.y + 22.0f), ImColor(255, 255, 255, 10), 11.0f);
        draw->AddCircleFilled(ImVec2(pillPos.x + 11.0f, pillPos.y + 11.0f), 3.5f, dotColor);
        ImGui::SetCursorScreenPos(ImVec2(pillPos.x + 21.0f, pillPos.y + (22.0f - ImGui::GetFontSize()) * 0.5f));
        ImGui::TextDisabled("%s", statusText.c_str());

        if (status == IRCStatus::ConnectionFailed) {
            ImGui::SameLine(0, 10.0f);
            ImGui::SetCursorPosY(13.0f);
            if (ImGui::Button("Retry", ImVec2(62.0f, 26.0f)))
                client.Connect(s_savedServer, 6667, s_savedUsername, s_savedPassword);
        }

        // Right side buttons
        ImGui::SetCursorPos(ImVec2(hw - 184.0f, 13.0f));
        if (ImGui::Button("Clear", ImVec2(64.0f, 26.0f)))
            client.ClearMessages();
        if (ImGui::IsItemHovered())
            ImGui::SetTooltip("Clear chat history");
        ImGui::SameLine();
        if (ImGui::Button("Log Out", ImVec2(84.0f, 26.0f))) {
            client.Disconnect();
            DeleteCredentials();
            s_savedUsername = "";
            s_savedPassword = "";
